...
// builtin Integer functions that must be friends
  ...
  friend int 		integer_sign(const Integer&);
  // returns -1, 0, +1
...
inline int integer_sign(const Integer& x)
{
  return (x.rep->len == 0) ? 0 : ( (x.rep->sgn == 1) ? 1 : -1 );
}

// I suspect that GCC has some issues with declaring
// template specialization to be a friend of a class -
// when I add template instantiation wit hthe same template arguments
// earlier in the file, GCC does not complain
// So I use a helper-function integer_sign to implemented
// the functionality and than wrap in template specialization
// in its simplest form, mentioning it only once,
// so GCC has no reasons to generate instantiation (just to be safe)
template<>
inline int sign<const Integer&>(const Integer& x)
{
  return integer_sign(x);
}
