...
// builtin Rational functions
  ...
  friend int       rational_sign(const Rational& x);             // -1, 0, or +1
...
inline int rational_sign(const Rational& x)
{
  	return sign(x.num);
}

// I suspect some GCC-problems in declaring template specialization
//to be a friend of a class - when I declare template _instantiation_
//for the same type arguments earlier in the file, GCC does not complain
template<>
inline int sign<const Rational&>(const Rational& x)
{
	return rational_sign(x);
}
