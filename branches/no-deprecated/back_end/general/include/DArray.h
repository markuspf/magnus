...
public:
  ...
  bool readFrom( std::istream& istr, Chars& errMesg ) 
  {
    MatrixRow<R>* matrix;
...
template <class R>
std::ostream& operator < ( std::ostream& ostr, const DArray<R>& DA )
{
  DA.look()->write(ostr);
  return ostr;
}
...
template <class R>
std::istream& operator > ( std::istream& istr, DArray<R>& DA)
{
  DA.change()->read(istr);
  return istr;
}
...
public:
 ...
 // the bounds are changed, but fisicaly memory does not realocating.
  ...
  friend IStreamPoll operator >> ( std::istream& istr, DArray& M )
  {
    Chars errMesg;
  ...
  inline friend std::ostream& operator << ( std::ostream& o, const DArray& v ) {
 
        if (v.height()==0 || v.width()==0)   {o << "| |"<<std::endl; return o;}

        for ( int i = 0; i < v.height(); i++ ){
              o << "| ";
              for (int j=0;j<v.width();j++)
                       o << v[i][j] << ", ";
              if (i==v.height()-1) { o << "|."<<std::endl;}
               else {o <<"|"<< std::endl;};
        }
        return o;
    }
  ...
  friend std::ostream& operator < <R>( std::ostream& ostr, const DArray& DA );
  
  friend std::istream& operator > <R>( std::istream& istr, DArray& DA);
...
public:
  ...
  DArrayParser(std::istream &istr) : WordParser(istr) { }
