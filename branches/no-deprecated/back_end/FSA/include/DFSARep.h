...
// that, and if they don't match the DFSA's cannot be equal.
  ...
  virtual void printAccepting(ostream &str=std::cout) const ;
  void printFlags(ostream &str=std::cout) const ;
  virtual void printStates(ostream &str=std::cout) const ;
  void printTableDensely(ostream &str=std::cout) const ;
  void printTableSparsely(ostream &str=std::cout) const ;
...
//  Bool nextAcceptedWord(Word w, int*& history) const {}; //@rn bad arg
  void readFrom(istream &str = std::cin);
  void printOn(ostream &str = std::cout) const;
  void GAPprintWord(ostream &str,const Word & w) const;
  void printAlphabet(ostream &str=std::cout) const ;
