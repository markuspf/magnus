...
if {[file executable $executable]} {
  if {[makeTranscript] == 1} {
    #set sessionManager [open "| $executable $magnus_home $tmp_dir 2>/dev/null" r+]
	 set sessionManager [open "| $tcl ./copier.tcl \"$executable $magnus_home $tmp_dir\" from-frontend.log from-backend.log 2>./session.log" r+]
  } else {
    #set sessionManager [open "| $executable $magnus_home $tmp_dir 2>/dev/tty" r+]
	 set sessionManager [open "| $tcl ./copier.tcl \"$executable $magnus_home $tmp_dir\" from-frontend.log from-backend.log 2>/dev/tty" r+]
  }
