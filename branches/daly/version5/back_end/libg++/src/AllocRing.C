...
// 20030628000 TPD cast the void* to an AllocNode
void AllocRing::clear()
{
  for (int i = 0; i < n; ++i)
  {
    if (nodes[i].ptr != 0)
    {
      // TPD deleting void* is undefined.
      //      delete(nodes[i].ptr);
      AllocQNode* p = (AllocQNode*)nodes[i].ptr;
      delete(p);
      nodes[i].ptr = 0;
    }
    nodes[i].sz = 0;
...
// 2003062800` TPD cast the void* to an AllocQNode
void AllocRing::free(void* p)
{
  ...
  if (idx >= 0)
  {
    ...
    AllocQNode* p = (AllocQNode*)nodes[idx].ptr;
    delete(p);
    nodes[idx].ptr = 0;
  }
}
