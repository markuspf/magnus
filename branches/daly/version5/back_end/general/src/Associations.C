 ...
 void AssociationsIteratorRep<Key, Val>::write( ostream& ostr ) const
{
  int curPoint = 1;
  int savCur=1;
  typename AssociationsRep<Key,Val>::CellType* from;
  from = theAssociations.look()->theList->nextCell;
  while ( from ) {
        curPoint++;
...
template <class Key, class Val>
 void AssociationsIteratorRep<Key, Val>::read( istream& istr ) {
     ...
     istr > curPos;   
     typename AssociationsRep<Key,Val>::CellType* from;
     from = theAssociations.look()->theList;
     for (int i=1; i<curPos;i++) from = from->nextCell;
     delete current;
     current = new typename AssociationsRep<Key,Val>::CellType();
     current = from;
}
