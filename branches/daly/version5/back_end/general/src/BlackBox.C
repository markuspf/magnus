...
ListOf< BlackBox* > BlackBox_known_BBs;//@am Compiler complains about not
...

{
  ...
  // 20031103 tpd force coerce '(void*)(int)'
  void* oldsig = (void *)(int)signal(sig, SIG_DFL);
...
void BlackBox::initialize(const Chars& startCommand, const Chars& restartCommand)
{
  ...
  // 20031103 tpd force coerce '(void*)(int)'
  ptr = (void *)(int)signal(SIGTERM, BlackBox_signal_handler);
  #ifdef DEBUG
    if ((int)ptr == -1) perror("BlackBox::BlackBox: signal");
  #endif

  // 20031103 tpd force coerce '(void*)(int)'
  ptr = (void *)(int)signal(SIGINT, BlackBox_signal_handler);
  #ifdef DEBUG
    if ((int)ptr == -1) perror("BlackBox::BlackBox: signal");
  #endif

  // 20031103 tpd force coerce '(void*)(int)'
  ptr = (void *)(int)signal(SIGHUP,  BlackBox_signal_handler);
  #ifdef DEBUG
    if ((int)ptr == -1) perror("BlackBox::BlackBox: signal");
  #endif
}
...
Bool BlackBox::doStart(const Chars& command, const char* greeting)
{
  ...
  if ( err ) {
    #ifdef DEBUG
      error("BlackBox::doStart: error in system call");
    #endif
    return FALSE;
  }
