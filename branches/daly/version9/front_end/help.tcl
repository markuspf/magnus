...
proc fetchHelpPage {pageSpec {pageTitle ""} {makeEdge 1}} {
  ...
  global magnus_home tcl_path helpPageLinkMap helpPageStack helpTraversal manualType manualFileName
# this is useless in stastistics
  set pageTitle "" 
  ...
  set htmlFile 0
  if { [regexp {(.+)\.(.+)} $file d name ext] == 1 } {
      #      if { [string $ext "dvi"] == 0} {
      #	  set dviFile 1
      #      } else if {  [string $ext "html"] == 0 } {
      set htmlFile 1
      #      }
      set path "$tcl_path/help/$file"
  } else {
...
#  if { $dviFile } {
#      # if dvi file proceed calling TkDVI 
#      helpDVI $path $title
#      helpQuit
#      return
#  } else 
  
  set manualFileName $path

  if { $htmlFile } {
      selectHelpType html
      helpHTML .help.frame.frame1.html $path 
  } else {
      selectHelpType text
  ...
  # end separation  of dvi files from regular helps
...
proc showHelp {{pageSpec "root::The CAISS-STAT Manual"} {title ""}} {
  ...
  wm title $w "CAISS-STAT Manual"
  wm iconname $w "CAISS-STAT Manual"
#@rn  wm minsize $w 400 400
  ...
  # @am thess two  widgets will alternate depending on the type of the help
  # text
  ...
  html $w.frame.frame1.html -bg white -imagecommand ImageCmd -unvisitedcolor MediumSeaGreen \
      -underlinehyperlinks 0 -yscrollcommand "$w.frame.frame1.s set" 
  ...
  button $w.frame.frame2.prt -relief raised -bd 2 -text "Print" -width 12 \
     -command "helpPrint"
...
## @am this is to enable switching between text and html help widgets

proc selectHelpType { t } {
    global manualType 

    if { [info exists manualType] } {
	if { $t != $manualType} {
	    #puts "Swithcing from $manualType to $t"
	    pack forget .help.frame.frame1.$manualType
	}
    } 
    
    set manualType $t
    
    pack .help.frame.frame1.$t -side top -fill both -padx 2m -pady 1m -expand 1
    .help.frame.frame1.s configure  -command ".help.frame.frame1.$t yview"

}

proc helpPrint {} {
    global manualType tmp_dir manualFileName
    
    set header "-XCAISS-STAT Manual."

    if { $manualType == "html"} {
	set ps_name [string range $manualFileName 0 [string last "." $manualFileName] ]
	set ps_name "$ps_name\ps"
	if { [catch {exec  mpage -1 -P $header $ps_name} ] } {
	    alert "Cannot print the Manual page"
	}
    } else {
	set textStr [.help.frame.frame1.$manualType get 0.0 end]
	...
	set out_f_name [string range $manualFileName [string last "/" $manualFileName] end]
	set outF [open "$tmp_dir/$out_f_name.prt" "w"]
	foreach  l $textList {
	...
	close $outF
	if { [catch { exec mpage -1 -P $header $tmp_dir/$out_f_name.prt }]} {
	    if { [catch {exec lpr $tmp_dir/$out_f_name.prt} ] } {
		alert "Cannot print the Manual page"
	    }
	} 
