...
bool SessionManager::startSocketServer(){
  try {m_pServerSocket=new ServerSocket(30000);
      return true;
...
void SessionManager::start( )
{
  ...
  if(!startSocketServer())
      error("The socket can not create!");
  while ( 1 ) {
	 readMessages();
...
/*  static Timer timer( 200 );
  ...
  m_pClientSocket=new ServerSocket();
...
{
  cerr << "Session Manager took control 1" << endl;

  TheDatabaseManager.takeControl();

  cerr << "Session Manager took control 2" << endl;

  if( quitRequested ) {
    static bool userWarned = false;

    cerr << "request quiting" << endl;
...
ServerSocket* SessionManager::m_pServerSocket=NULL;
ServerSocket* SessionManager::m_pClientSocket=NULL;
