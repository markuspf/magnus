...
#if SAFETY > 0
  if ( !bHaveCyclicDecomposition )
    error("the abelianization was not done before AbelianGroupRep"
	  "::makeSubgroupPresentation(const VectorOf<Word>& vG) const ");
#endif

  if (vG.length()==0 || theGroup.numberOfGenerators()==0)
    return  AbelianSGPresentationRep(AbelianGroup((AbelianGroupRep*)this),
				     vG,FPGroup(),Matrix<Integer>(), true);  
  
  char s[10];
  Matrix<Integer> presentMatrix(vG.length(),
				rankOfFreePart+theInvariants.length());

   for (int j = 0; j< vG.length(); j++){
     presentMatrix[j] = oldToNewGens(oldInAbelianForm(vG[j])).getPowers();
   }
   GaussTransformation<Integer>* gTrans = 
     new GaussTransformation<Integer>(presentMatrix);
   
   gTrans->startComputation();
   gTrans->run();
   presentMatrix =gTrans->getMatrix(); 
   // If there are no torsion part, build a new group with generators only.   
   if (theInvariants.length()==0) {
       return buildTorsionFreePresentation(presentMatrix,vG);
   }   
  
   int savTPos;
   bool isFound = false;
   
   // Find the first dependent generator in matrix
   for ( savTPos=0;savTPos<gTrans->getMatrix().height();savTPos++){
     if (isAllZero(0,rankOfFreePart-1,gTrans->refMatrix()[savTPos]))
       if (!isAllZero(rankOfFreePart,gTrans->refMatrix().width()-1,
		      gTrans->refMatrix()[savTPos])){
	 isFound = true;
	 break;
       }   
   }
   
   // If not found, returns the group witout relations.
   if (!isFound){
     return buildTorsionFreePresentation(gTrans->refMatrix(),vG);
   }
   
   // Trancate independent part
   if (rankOfFreePart>0) gTrans->refMatrix().colDelRange(0,rankOfFreePart-1);
   if (savTPos>0) gTrans->refMatrix().rowDelRange(0,savTPos-1); // Trancate the matrix
   if (gTrans->refMatrix().width()>0){
     gTrans->startComputation();
     gTrans->run();
   }
   VectorOf<Integer> transCoef(theInvariants.length()); 
   // Coefficients of rows multiply
   // Matrix of new generators
   //   Matrix<Integer> newSubGenerators(vG.length()-savTPos,theInvariants.length());
   Matrix<Integer> newSubGenerators(theInvariants.length(),theInvariants.length());
   
   Integer l;
   int index=0;
   while (index < newSubGenerators.height() && gTrans->refMatrix().width()>0){
     l = theInvariants[index]/gcd(theInvariants[index], 
				  gTrans->refMatrix()[0][0] % 
				  theInvariants[index]);
     
     for (int i=0;i<gTrans->refMatrix().width();i++){
       newSubGenerators[index][newSubGenerators.width() - 
			      gTrans->refMatrix().width()+i] =
	 gTrans->refMatrix()[0][i];
       gTrans->refMatrix()[0][i] = 
	 (gTrans->refMatrix()[0][i]*l)%theInvariants[i+index];
     }
     transCoef[index]=l;
     index++;
     gTrans->refMatrix().colDelRange(0,0,false);
     if (gTrans->refMatrix().width()>0){
       //          newSubGenerators.rowInsAfter(newSubGenerators.lastRow(),1);
       gTrans->startComputation();
       gTrans->run();
     }
   }
   delete gTrans;
   // Here we are building relations
   Matrix<Integer> relationCoef(newSubGenerators.height()-1,
				newSubGenerators.width()-1);
   //Matrix of relation cofficients
   
   VectorOf<Integer> tmpVect(newSubGenerators.width()-1);
   int numOfRelation = min(relationCoef.width(),relationCoef.height());
   for (int relNum=0;relNum<newSubGenerators.height()-1;relNum++){
     for (int i=0;i<tmpVect.length()-relNum;i++){
       tmpVect[i]=(transCoef[relNum]*newSubGenerators[relNum][relNum+i+1]) % 
	 theInvariants[i+relNum+1];
     }
     for (int i=0; i<numOfRelation-relNum;i++){
       for (relationCoef[relNum][i]=0;relationCoef[relNum][i]<
	      theInvariants[i+relNum+1];relationCoef[relNum][i]+=1){
	 if ((relationCoef[relNum][i] * 
	      newSubGenerators[relNum+i+1][relNum+i+1]-tmpVect[i])%
	     theInvariants[i+relNum+1] == 0) {
	   break;
	 }
       }
       for (int j=0;j<tmpVect.length()-relNum;j++){
	 tmpVect[j] = tmpVect[j] - (relationCoef[relNum][i]*
				    newSubGenerators[relNum+i+1][j+relNum+1])%
	   theInvariants[i+relNum+1];
       }
     }
   }
   int nGens=0;
   int nGensOfFreePart = 0;
   SetOf<Word> nRelators;
   VectorOf<Chars> genNames(savTPos+newSubGenerators.height());
   DArray<Integer> presentGens(savTPos+newSubGenerators.height(),
			       rankOfFreePart+theInvariants.length());
   int numOfZeroGens = 0;
   // Here we are building the group with our presentation
   // Generators of free part
   for (int i=0;i<savTPos;i++)
     if (!isAllZero(0,presentMatrix.width()-1,presentMatrix[i])){
       sprintf(s,"s%u",nGens+1);
       genNames[nGens] = s;
       presentGens[nGens] = presentMatrix[i];
       nGens++;
       nGensOfFreePart++;
     }
   // Generators of torsion part and relators
   VectorOf<Generator> vGen;
   AbelianWord ab;
   for (int i=0;i<newSubGenerators.height();i++)
     if (!isAllZero(0,newSubGenerators.width()-1,newSubGenerators[i])){
       sprintf(s,"s%u",nGens+1);
       genNames[nGens] = s;
       for (int j=0;j<theInvariants.length();j++)
	 presentGens[nGens][j+rankOfFreePart]=newSubGenerators[i][j];
       // Here begins a part of building a relator
       for (int j=0;j< abs(transCoef[i]);j++)
	 if (transCoef[i]<0)
	   vGen.append(-1*(1+nGens));
	 else
	   vGen.append(nGens+1);
       if (i<relationCoef.height()){
	 for (int j=0;j<relationCoef.width()-i;j++){
	   if ( relationCoef[i][j]!=0)
	     for (int j1=0;j1< abs(relationCoef[i][j]);j1++){
	       if (relationCoef[i][j]>0)
		 vGen.append(-1*(nGens+j+2));
	       else
		 vGen.append(nGens+j+2);
	     }
	 }
       }
       nGens++;
       nRelators.adjoinElement(Word(vGen));
       vGen = VectorOf<Generator>();
     }  
   // We need to have positive numbers of powers in torsion part
   for (int i = rankOfFreePart;i<presentGens.width();i++) 
     for (int j=0;j<presentGens.height();j++){
       presentGens[j][i] = presentGens[j][i]%theInvariants[i-rankOfFreePart];
       if (presentGens[j][i]<0)
	 presentGens[j][i]+=theInvariants[i-rankOfFreePart];
     }
   
   genNames.shrink(nGens);
   if (nGens<presentGens.height())
     presentGens.rowDelRange(nGens,presentGens.lastRow(),false);
   FPGroup thePresentation(genNames,nRelators);
   return  AbelianSGPresentationRep(AbelianGroup((AbelianGroupRep*)this),
				    vG,thePresentation,
				    presentGens, true);  
}

AbelianSGPresentationRep AbelianGroupRep::buildTorsionFreePresentation
( Matrix<Integer>& gensTransformation, const VectorOf<Word>& vG)const
{
  int nGens=0;
  char s[10];
  int numOfZeroGens = 0;
  DArray<Integer> presentGens(gensTransformation.height(),
			      gensTransformation.width());
  VectorOf<Chars> genNames(gensTransformation.height());
  for (int i=0;i<gensTransformation.height();i++)
    if (!isAllZero(0,gensTransformation.width()-1,gensTransformation[i])){
      presentGens[nGens] = gensTransformation[i];
      sprintf(s,"s%u",nGens+1);
      genNames[nGens] = s;
      nGens++;
    }   
  ...
  return  AbelianSGPresentationRep(AbelianGroup((AbelianGroupRep*)this),vG,
				   thePresentation,presentGens,true);  
}

bool  AbelianGroupRep::isAllZero(int from, int to, MatrixRow<Integer>& vc) const{
#if SAFETY > 0
  if (from<0 || to >= vc.length() )
    error("wrong parameters in AbelianGroupRep::isAllZero(int,int,MatrixRow<Integer>)");
#endif  
...
Integer AbelianGroupRep::pHeightOfElt(const Word& w, const Integer& p,
				      Integer orderofElt) const
{
#if SAFETY > 0
  if ( !primeBasisFound )
    error("The primary decomposition was not done before the call of"
	  " AbelianGroupRep::pHeightOfElt(Word,Integer,Integer)");
#endif
  Integer orderOfW; 
  
  // This done in case if you have already the order of the elt, 
  // to avoid new computation
  if (orderofElt < 0)
    orderOfW = orderOfElt(w);
  else
    orderOfW = orderofElt;
  ...
  if(!rankOfFreePart)
    ...
    for (int i=1;i<rankOfFreePart;i++)
      d = gcd(d,elemInPrime[i]);
    m = IntProblems().powerOf(d,p);
    if (m<0) m=0;
  }
  
  if (IntProblems().powerOf(orderOfW,p)!=-1 && orderOfW!=0){
    bool foundP = false;
    int pHeight;
    Integer k;
    Integer savPower;
    ...
    bool isFirst = true;
    for (int i=0;i<primeBasicMatrix.height();i++){
      if (p==primeBasicMatrix[i][PRIMES]){
	if (elemInPrime[i+rankOfFreePart]!=0){
	  k = elemInPrime[i+rankOfFreePart] - elemInPrime[i+rankOfFreePart]%p;
	  if (k>=p){
	    while ((savPower=IntProblems().powerOf(k,p))==-1 ){
	      k = k - p;
	    }
	    if (isFirst){
	      pHeight = savPower.as_long();
	      isFirst = false;
	    }    
	    else 
	      pHeight=min(savPower.as_long(),pHeight);
	  }
	  else {
	    pHeight=0;
	    isFirst = false;
	...
	return Integer(min(m.as_long(),pHeight));
  }
  else
    return m;
}
...
#if SAFETY > 0
  if ( !bHaveCyclicDecomposition )
    error("the abelianization was not done before AbelianGroupRep::findSubgIntersection( const VectorOf<Word>& , const VectorOf<Word>& , File& )");
#endif
...
bool AbelianGroupRep::isEltProperPower(const Word& w) const
{
  ...
  if ( haveTorsion ) {
    ...
    for (int i=0;i<pDecOfg.height();i++)
      if (pHeightOfElt( w,pDecOfg[i][0],gOrder) > 0 )   
	return true;
    return false;
  } else {
...
void AbelianGroupRep::abelianMaximalRoot(const Word& w, Word& maxRoot, Integer& maxExp) const
{
  ...
  if ( haveTorsion ) {
    ...
    for (int i=0;i<pHeights.length();i++){
      pHeights[i] = pHeightOfElt( w,pDecOfd[i][0],gOrder);
      if (pHeights[i] < 0) 
	alfas[i] = pDecOfd[i][1];
      else
