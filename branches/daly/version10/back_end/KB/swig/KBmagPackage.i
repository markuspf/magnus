...
public:
  ...
  %name(KBmagPackageFromWordOrder) KBmagPackage(const VectorOf<Chars>& genNames, const SetOf<Word>& rels,
          const WordOrder & word_order) ; 

  %name(KBmagPackageFromAllParam)  KBmagPackage(const VectorOf<Chars>& genNames, const SetOf<Word>& rels,
...
// with the automata, e.g. hyperbolicity testing.
  ...
  %name(minimizeFromDFSA) Bool minimize(DFSA & D);
  %name(minimizeFromGroupDFSA)  Bool minimize(GroupDFSA & D);
