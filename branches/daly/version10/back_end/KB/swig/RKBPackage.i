...
public:
  ...
  %name(RKBPackageFromWordOrder) RKBPackage(const VectorOf<Chars>& genNames, const SetOf<Word>& relators,
...
//  void printRules(ostream& str = cout);
  ...
  %name(differenceMachineFromSet) DiffMachine differenceMachine(const SetOf<Word> & differences);
