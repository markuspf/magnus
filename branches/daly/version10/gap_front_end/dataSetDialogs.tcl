...
proc defineAttibuteDataSetObjectDialog {message dialog title name helps defn \
			 errorMsg cathegory {dimension 1} {cursorPos "1.2"} } {
  ...
  button $w.frame3.frame.string -foreground blue -text "Enter from Proportion Testing Format" -relief flat \
             -command "defineDataSetObjectDialog2 $w \"$message\" $dialog \"$title\" $name \"$helps\" \"$defn\" \
			 \"$errorMsg\" \"$cathegory\" 2 $cursorPos \"SPREAD\" "
  button $w.frame3.frame.spread -foreground blue -text "Enter from Contingency Format" -relief flat \
             -command "defineDataSetObjectDialog2 $w \"$message\" $dialog \"$title\" $name \"$helps\" \"$defn\" \
			 \"$errorMsg\" \"$cathegory\" $dimension $cursorPos \"SPREAD\" "
  button $w.frame3.frame.exitfile -foreground blue -text "Enter from an Existing File" -relief flat \
             -command "defineDataSetObjectDialog2 $w \"$message\" $dialog \"$title\" $name \"$helps\" \"$defn\" \
