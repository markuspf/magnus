...
proc append_viewLog {viewID msg} {
  ...
  set message $msg
# changed by xuxw to show link on dialog not log
...
# this file has been changed to show file using showHelp
#instead of fileView
...
proc insert_table_to_log {logName tableExp} {
   postToLog $tableExp  
   for {set i 0} {$i<[llength [lindex $tableExp 0]]} {incr i} {
      set column_width($i) 0
   }

   foreach rowExp $tableExp {
      for {set i 0} {$i<[llength $rowExp]} {incr i} {
         set string_length [ string length [lindex $rowExp $i]]
         if {$string_length>$column_width($i)} {
            set column_width($i) $string_length
         }
     }
  }
  foreach rowExp $tableExp {
      set pos 0
      for {set i 0} {$i<[llength $rowExp]} {incr i} {
        $logName insert end [lindex $rowExp $i]
        $logName insert end " "
 #       postToLog "c1: $column_width($i) c2: [llength [lindex $rowExp $i]]"
        for {set j [ string length [lindex $rowExp $i]]} {$j<$column_width($i)} {incr j} {
...
proc viewLogInsert {logName msg} {
  ...
  if { [regexp -indices {@start_table(.*)@end_table} $message \
	       exp indices textIndices] > 0 } {
      set leftIndex  [lindex $exp 0]
      set rightIndex [lindex $exp 1]
      set tableExp [string range $message $leftIndex $rightIndex]
      regsub @start_table $tableExp "" tableExp
      regsub @end_table $tableExp "" tableExp
      logScrollAppend $logName [string range $message 0 [incr leftIndex -1]]
      insert_table_to_log $logName $tableExp
      logScrollAppend $logName [string range $message [incr rightIndex] end]
...
proc fileView { linkNumber fileName {title " "} } {
  ...
  makeEditMenu $w.mbar.edit read-only
  menubutton $w.mbar.help -text Help -state disabled
...
proc handleRSApproximation { linkNumber fileName } {

    fileView $linkNumber $fileName \
	{L's approximation}
}

proc updateRSApproximation { windowName {message ""} {tagList ""} } {

  logScrollAppend $windowName.logFrame.log $message $tagList
}

proc handleFurtherAnylysis { linkNumber fileName } {

#    fileView $linkNumber $fileName \
#	"Further Analysis"
#   TexFileView "$fileName.dvi"
    showHelp "$fileName\:\: " " "
}

proc TexFileView {fileName} {
    showDviHelp {} $fileName
#   destroy .tkdviHelp
}
