...
proc getDefnFromFile_Button { butn textw } {
    getDefnFromFile $textw
    $butn configure -state normal
}

proc getDefnFromDatabase_Button { butn cathegory entry textw } {
...
proc makeDefnHelp {w helpKind {blink_p 0}} {
  if {![winfo exists $w.frame4]} {
    ...
    label $w.frame4.label -text "  "
    text $w.frame4.frame.text -relief sunken -bd 2 -height 7 -width 50 \
    ...
    pack $w.frame4 -fill both -expand 1 -after $w.frame3
  }
...
proc defineObjectDialog_OK {w dialog title message} {
  ...
  puts [get_transcriptFD] "xxw begin"
  puts [get_transcriptFD] $message
  puts [get_transcriptFD] "xxw end"
...
proc defineDataSetDialog {title message {name ""} {defn ""} {errorMsg ""}} {
  if {$name == ""} { set name [uniqueObjectName S] }

  if {$defn == ""} { set defn "{     }" }

  defineObjectDialog $message defineDataSetDialog $title $name \
    {{{Data set format} DataSetConventions}} \
    $defn $errorMsg {DATA_SET}
}

proc defineOneVariableDataSetDialog {title message {name ""} {defn ""} {errorMsg ""}} {
  if {$name == ""} { set name [uniqueObjectName S] }

  if {$defn == ""} { set defn "{     }" }

  defineDataSetObjectDialog $message defineOneVariableDataSetDialog $title $name \
    {{{About One Variable Data Sets} OneVariableDataSetConventions}  \
     {{Check-in in Spreadsheet Format} OneVariableDataSetSpreadsheetFormat} \
     {{Check-in in String Format} OneVariableDataSetStringFormat} \
     {{Check-in from an Existing File/One Variable} OneVariableDataSetFromFile} \
     {{Check-in from the CAISS-Stat Database} OneVariableDataSetFromDatabase} \
    } \
    $defn $errorMsg {DATA_SET}
}

proc defineTwoVariableDataSetDialog {title message {name ""} {defn ""} {errorMsg ""}} {
  if {$name == ""} { set name [uniqueObjectName S] }

  if {$defn == ""} { set defn "{     } {     }" }

  defineDataSetObjectDialog $message defineTwoVariableDataSetDialog $title $name \
    {{{About Two Variable Data Sets} TwoVariableDataSetConventions}  \
     {{Check-in in Spreadsheet Format} TwoVariableDataSetSpreadsheetFormat} \
     {{Check-in in String Format} TwoVariableDataSetStringFormat} \
     {{Check-in from an Existing File/Two Variable} TwoVariableDataSetFromFile} \
     {{Check-in from the CAISS-Stat Database} TwoVariableDataSetFromDatabase} \
    } \
    $defn $errorMsg {DATA_SET} 2
}

proc defineMultiVariableDataSetDialog {title message {name ""} {defn ""} {errorMsg ""}} {
  if {$name == ""} { set name [uniqueObjectName S] }

  if {$defn == ""} { set defn "{     }" }

  defineMultiDataSetObjectDialog $message defineDataSetDialog $title $name \
    {{{Multi Data Set CheckIn} MultiDataSetCheckIn}} \
    $defn $errorMsg {DATA_SET}
}


proc defineCrossClassifiedDataSetDialog {title message {name ""} {defn ""} {errorMsg ""}} {
  if {$name == ""} { set name [uniqueObjectName S] }

  if {$defn == ""} { set defn "{     }" }

  defineAttibuteDataSetObjectDialog $message defineDataSetDialog $title $name \
    {{{ About Attribute Data Sets} AttributeDataSetConventions} \
     {{Proportion testing Format} AttributeDataSetSProportionFormat} \
     {{Contingency Format} AttributeDataSetContingencyFormat} \
     {{Check-in from an Existing File/Attribute} AttributeDataSetFromFile} \
    } \
    $defn $errorMsg {DATA_SET} 100
}


proc defineTimeSeriesDataSetDialog {title message {name ""} {defn ""} {errorMsg ""}} {
  if {$name == ""} { set name [uniqueObjectName S] }

  if {$defn == ""} { set defn "{     }" }

  defineDataSetObjectDialog $message defineDataSetDialog $title $name \
    {{{Data set format} DataSetConventions}} \
    $defn $errorMsg {DATA_SET}
}


proc defineQualityControlDataSetDialog {title message {name ""} {defn ""} {errorMsg ""}} {
  if {$name == ""} { set name [uniqueObjectName S] }
...
proc defineProbabilityDistributionDialog {title message {name ""} {defn ""} {errorMsg ""}} {
  if {$name == ""} { set name [uniqueObjectName S] }

#  if {$defn == ""} { set defn "{     }" }

  defineProbabilityDataSetDialog $message $title 

}

proc defineDescriptionOfDataSetDialog {title message {name ""} {defn ""} {errorMsg ""}} {

  if {$name == ""} { set name [uniqueObjectName w] }
#xxw comment add a new dialog

  defineDataSetObjectDialog $message defineWordDialog $title $name \
    {{{Word format} wordConventions}} $defn $errorMsg {WORD}
}

proc defineMeanAnalysisDialog {title message {name ""} {defn ""} {errorMsg ""}} {
  defineDataSetMeanAnalysisDialog  $message $title
}

# -------------------------- CALCULUS --------------------------------- #

proc definePolynomialDialog {title message {name ""} {defn ""} {errorMsg ""}} {
  if {$name == ""} { set name [uniqueObjectName P] }

  if {$defn == ""} { set defn "" }

  defineObjectDialog $message defineDataSetDialog $title $name \
    {{{Polynomial format} PolynomialConventions}} \
    $defn $errorMsg {POLYNOMIAL}
}

# @db this is very temporary
proc showPolynomialGraph { name } {
    open "|/usr/bormotov/tclex/bin/bltwish < /usr/bormotov/magnus/front_end/proba/blttest.tcl"
}
