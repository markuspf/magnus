...
proc init_graphicView {icon viewParameters} {
  extractArguments {viewID viewTitle viewBanner totalARCs helpID subproblems heritage} \
                   $viewParameters
   set_viewID $icon $viewID
   set_viewIcon $viewID $icon
   set_viewTitle $viewID "Graphical Analysis"
   set_viewBanner $viewID "Graphical analysis"
   set_viewHeritage $viewID $heritage
}

proc init_regressionGraphicView {icon viewParameters} {
  extractArguments {viewID viewTitle viewBanner totalARCs helpID subproblems forms} \
                   $viewParameters

  set_viewID $icon $viewID
  set_viewIcon $viewID $icon
  set_viewTitle $viewID "Regression Graphical Analysis"
  set_viewBanner $viewID "Regression Graphical Analysis"
  set_viewParameter $viewID $forms
}

proc init_resultView {icon viewParameters} {
  extractArguments {viewID viewTitle viewBanner totalARCs helpID subproblems heritage} \
                   $viewParameters

  set_viewID $icon $viewID
  set_viewIcon $viewID $icon
  set_viewTitle $viewID $heritage
  set_viewBanner $viewID $heritage
  set_viewLog $viewID {}
  set_viewHeritage $viewID $heritage
...
proc init_summaryView {icon viewParameters} {
  extractArguments {viewID viewTitle viewBanner totalARCs helpID subproblems heritage} \
                   $viewParameters

  set_viewID $icon $viewID
  set_viewIcon $viewID $icon
  set_viewTitle $viewID "Data Summary"
  set_viewBanner $viewID "Data Summary"
  set_viewHeritage $viewID $heritage
}

proc init_fitView {icon viewParameters} {
  extractArguments {viewID viewTitle viewBanner totalARCs helpID subproblems} \
                   $viewParameters

  set_viewID $icon $viewID
  set_viewIcon $viewID $icon
  set_viewTitle $viewID "Goodness Of Fit"
  set_viewBanner $viewID "Goodness Of Fit"
  set_viewLog $viewID {}

}

proc init_ODVSReportView {icon viewParameters} {
  extractArguments {viewID viewTitle viewBanner totalARCs helpID subproblems forms} \
                   $viewParameters

  set_viewID $icon $viewID
  set_viewIcon $viewID $icon
  set_viewTitle $viewID "Dataset Report"
  set_viewBanner $viewID "Dataset Report"
  set_viewParameter $viewID $forms
  set_viewLog $viewID {}

}

proc init_TVDSReportIndependentView {icon viewParameters} {
  extractArguments {viewID viewTitle viewBanner totalARCs helpID subproblems forms} \
                   $viewParameters
...
proc init_TVDSReportRegressionView {icon viewParameters} {
  extractArguments {viewID viewTitle viewBanner totalARCs helpID subproblems forms} \
                   $viewParameters

  set_viewID $icon $viewID
  set_viewIcon $viewID $icon
  set_viewTitle $viewID "Dataset Regression Report"
  set_viewBanner $viewID "Dataset Regression Report"
  set_viewParameter $viewID $forms
  set_viewLog $viewID {}
}

proc init_residualView  {icon viewParameters} {
  extractArguments {viewID viewTitle viewBanner totalARCs helpID subproblems forms} \
                   $viewParameters

  set_viewID $icon $viewID
  set_viewIcon $viewID $icon
  set_viewTitle $viewID "Residual Analysis"
  set_viewBanner $viewID "Residual Analysis"
  set_viewParameter $viewID $forms
  set_viewLog $viewID {}

}

proc init_probDistGraphView {icon viewParameters} {
  extractArguments {viewID viewTitle viewBanner totalARCs helpID subproblems data} \
                   $viewParameters

  set_viewID $icon $viewID
  set_viewIcon $viewID $icon
  set_viewTitle $viewID "Distribution Graph"
  set_viewBanner $viewID "Distribution Graph"
  set_viewParameter $viewID $data
  set_viewLog $viewID {}
}

proc init_probDistTableView {icon viewParameters} {
  extractArguments {viewID viewTitle viewBanner totalARCs helpID subproblems data} \
                   $viewParameters

  set_viewID $icon $viewID
  set_viewIcon $viewID $icon
  set_viewTitle $viewID "Distribution Table"
  set_viewBanner $viewID "Distribution Table"
  set_viewParameter $viewID $data
  set_viewLog $viewID {}
}

proc init_probDistSimulateView {icon viewParameters} {
  extractArguments {viewID viewTitle viewBanner totalARCs helpID subproblems data} \
                   $viewParameters
...
proc EnumertatorProblemView {viewID} {
  ...
  if { [have_probParameters $viewID]  } {
      menubutton $w.mbar.parameters -text Parameters -menu $w.mbar.parameters.menu
      menu $w.mbar.parameters.menu 
      $w.mbar.parameters.menu add command -label "Show parameters" \
...
proc editDatasetView_OK {w viewID} {
  global A
  set_viewPath $viewID {}
  set icon [get_viewIcon $viewID]
  set ID [get_objectID $icon]
  set type $A($ID,checkin_type)
  set type [expr $type-1]
  set name [get_objectName $icon]
  set defn [$w.defn get 1.0 end-1char]
  workspace_deleteSelection
  set message "-2 $type $name \{$defn\}"
  set dialog "defineDataSetObjectDialog"
  set title "untitled"
  set msg "-2 $type"
  save_pendingCheckin $name $dialog "$defn" $title $msg
  eval [concat toSessionManager "\"$message\""]
  destroy $w
}

proc objectViewSelection {} {
  global workspace_selection
  set iconID $workspace_selection
  set view [get_viewBuilder $iconID]
  set viewID [expr $iconID-1]
  eval "$view $viewID"
}
...
proc objectView {viewID} {
  ...
  menubutton $w.filemb -text File -menu $w.filemb.fileMenu
  makeEditMenu $w.editmb 
  menubutton $w.helpmb -text Help -menu $w.helpmb.helpMenu

  menu $w.filemb.fileMenu
    $w.filemb.fileMenu add command -label "Save Dataset As..." \
      -command "saveLogToFile $w.defn"

    $w.filemb.fileMenu add command -label "Save log..." \
      -command "saveLogToFile $w.log"


  menu $w.helpmb.helpMenu
  ...
  set defn [get_objectDefinition $icon]
  if { [string length $defn ] } {
...
#      $w.defn configure -state disabled
  ...
  button $w.cancel -text Close -relief raised -bd 2 -width 8\
    -command "destroy $w; set_viewPath $viewID {}"
  button $w.ok -text Edit -relief raised -bd 2 -width 8 \
           -command "editDatasetView_OK $w $viewID"
  if {[string range $defn 0 0]>="A" && [string range $defn 0 0]<="Z"} {
