...
proc makeWorkspace {} {
  ...
  wm geometry $w -18+20
#  set sizes [wm maxsize $w]
#  alert $sizes
#  set size1 [lindex $sizes 0]
#  set size2 [lindex $sizes 1]
#  wm minsize $w $size1 $size2
  # Put in menu bar
#  wm geometry $w ${size1}x${size2}+0+0
  frame $w.mbar -relief raised -bd 2
  pack $w.mbar -fill x

  menubutton $w.mbar.checkin -text "Check-in" -menu $w.mbar.checkin.menu \
    -padx 1m
  menubutton $w.mbar.tools -text "Tools" -menu $w.mbar.tools.menu -padx 1m

  menubutton $w.mbar.window -text "Window" -menu $w.mbar.window.menu -padx 1m

  menubutton $w.mbar.help -text Help -menu $w.mbar.help.menu -padx 1m

  pack $w.mbar.checkin $w.mbar.tools  -side left -padx 1m

  pack $w.mbar.checkin $w.mbar.window  -side left -padx 1m

  pack $w.mbar.help -side right -padx 1m

  menu $w.mbar.checkin.menu

  menu $w.mbar.tools.menu

  menu $w.mbar.window.menu

  menu $w.mbar.help.menu
    $w.mbar.help.menu add command -label "About the Workspace" \
  ...
  canvas $c -width 600 -height 500 -scrollregion {0 0 1000 1000} \
...
proc workspace_drawIcon {iconID name x y} {
  ...
  switch -exact $iconID {
   ...
   matrix {
     set def [get_pendingDefn $name]
     set ibegin [string first "\[\[" $def]
     set def [string  range $def $ibegin end]
     puts stderr "def: $def"
     set def [split $def "\]"]
     set col1 [lindex $def 0]
     set col1 [split $col1 ","]
     set col [llength $def]
     set col [expr $col-2]
     set row [llength $col1]    
     set r [$c create oval -8 -8 8 8 -tags icon]
     puts stderr "row: $row, col:$col"
     if {$row < $col} {
         set lleft [$c create line -20 -10 -20 10 -tags icon]
         set ltop  [$c create line -20 -10 20 -10 -tags icon]
         set lright [$c create line 20 -10 20 10 -tags icon]
         set lbottom  [$c create line -20 10 20 10 -tags icon]
         set row_text [$c create text  -25 0 -text $row]
         set col_text [$c create text  0 -15 -text $col]
     }
     if {$row == $col} {
         set lleft [$c create line -15 -15 -15 15 -tags icon]
         set ltop  [$c create line -15 -15 15 -15 -tags icon]
         set lright [$c create line 15 -15 15 15 -tags icon]
         set lbottom  [$c create line -15 15 15 15 -tags icon]
         set row_text [$c create text  -20 0 -text $row]
         set col_text [$c create text  0 -20 -text $col]
     }
     if {$row > $col} {
         set lleft [$c create line -10 -20 -10 20 -tags icon]
         set ltop  [$c create line -10 -20 10 -20 -tags icon]
         set lright [$c create line 10 -20 10 20 -tags icon]
         set lbottom  [$c create line -10 20 10 20 -tags icon]   
         set row_text [$c create text -15 0 -text $row]
         set col_text [$c create text 0 -25 -text $col]     
     }
      $c move $lleft $x $y
      $c move $ltop $x $y
      $c move $lright $x $y
      ...
      set_iconComponent $r $lleft
      set_iconComponent $r $ltop
      set_iconComponent $r $lright
      set_iconComponent $r $lbottom
      set_iconComponent $r $row_text
      set_iconComponent $r $col_text
      set_iconOfCanvasWidget $r $lleft
      set_iconOfCanvasWidget $r $ltop
      set_iconOfCanvasWidget $r $lright
      set_iconOfCanvasWidget $r $lbottom     
      set_iconComponent $r $row_text
      set_iconComponent $r $col_text
   }
  ...
  if {[info exists LINUX_OS]} {
    if {$iconID == "problem"} {
      set t [$c create text [expr $x + 2] [expr [lindex [$c coords $r] 3] + 13] \
       ...
       set new_name [split $name _]
       if {[llength $new_name]==2} {
           set part1 [lindex $new_name 0]
           set part2 [lindex $new_name 1]
           set t [$c create text [expr $x + 2] [expr [lindex [$c coords $r] 3] + 13] \
                         -text $part1 -font $font -tags part1]
           set font "*-times-medium-r-normal--*-90-*-*-*-*-*-*"
           set t2 [$c create text [expr $x + 5 +5*[string length $part1]] [expr [lindex [$c coords $r] 3] + 16] \
                         -text $part2 -font $font -tags part1]           
       } else {
           set t [$c create text [expr $x + 2] [expr [lindex [$c coords $r] 3] + 13] \
                         -text $name -font $font -tags name]
       }
...
#    set_edge $edge $result $i $d
#    low_edge $edge
#instead of magnus, we implemented for subtrees.
     set edge [create_treeEdge $x $y [get_iconXPosition $i] \
		  [get_iconYPosition $i] ]
     set_edge $edge $result $i $d
     low_edge $edge
  }
...
#  we changed this to make tree arrangement.
     set x1 [get_iconXPosition [get_objectIcon [lindex $dependencies 0]]]
     set y1 [get_iconYPosition [get_objectIcon [lindex $dependencies 0]]]
#    set x 70
#    set y 0
    for {set i 1} {$i<[expr $maxLevel*2]} {incr i} {
       set x [expr $x1-100*($i%2)+50]
       set y [expr $y1+25+25*$i] 
       if {$x >= $X1 && $y >= $Y1 && $x <= $X2 && $y <= $Y2 &&
            [$c find overlapping [expr $x - 21] [expr $y - 21] \
                        [expr $x + 21] [expr $y + 21]] == ""} {
            return
...
proc create_treeEdge { leafX leafY rootX rootY } {
  set c .workspace.frame2.c
  set l1 [$c create line $rootX $rootY $rootX $leafY]
  set l2 [$c create line $rootX $leafY $leafX $leafY]
  return [list $l1 $l2]
}
