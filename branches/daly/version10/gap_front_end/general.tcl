...
proc VerifyValueType {value type name} {

   if {$value==""} {
        alert "You must give a value of $name"
        return false
   }
# digit D
# number N
# number between 0 and 1  F
# positive digit P
  switch $type {
    D {
            if {[regexp {^([-]?[0-9]*[.]?[0-9]*)$} $value]==0} {
               alert "You must give a correct value of $name"
               return false
            }
      }
     N {
            if {[regexp {^([0-9]*)$} $value]==0} {
               alert "You must give a number of $name"
               return false
            }
      }
     F {
            if {[regexp {^([0]?[.][0-9]*)$} $value]==0} {
              alert "You must give a value of $name between 0 and 1"
              return false
            }
       }
    P {
            if {[regexp {^([0-9]*[.]?[0-9]*)$} $value]==0} {
               alert "You must give a positive value of $name"
               return false
            }
      }
  }
...
proc substitute_left_bracket { message new_char} {
  set new_message ""
  for {set i 0} {$i<[string length "$message"]} {incr i} {
      ...
      set new_message "$new_message$ch"
  }
  return $new_message
}
