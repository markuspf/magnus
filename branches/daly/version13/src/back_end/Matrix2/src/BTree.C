...
template <class K>
bool BTree<K>::search( const K& key, const BTreePage<K>& searchPage,
		       BTreePage<K> **keyPage, int& position )
{
  *keyPage = (BTreePage<K> *)&searchPage;
  position = 0;
...
void BTree<K>::insert( const K& key )
{
  ...
  if( search(key, *root, &page, position) ) {
    ...
    BTreePage<K> *leftLink = NULL;
    BTreePage<K> *rightLink = NULL;
	...
	BTreePage<K> *newBrotherPtr = new BTreePage<K>(theOrder);
...
template <class K>
bool BTree<K>::Delete( const K& key )
{
  BTreePage<K> *page;
  int position;
  if( search(key, *root, &page, position) ) {
...
template <class K>
void BTree<K>::deleteKey( BTreePage<K> *page, int position )
{
  ...
  while( page->numOfKeys < minNumOfKeys ) {
    ...
    BTreePage<K> *parentLink = page->parentLink;
    int numOfParentLink = page->numOfParentLink;
    int parentNumOfKeys = parentLink->numOfKeys;
    int numOfKeys = page->numOfKeys;
    bool isLeaf = ( page->links[0] == NULL );
    
    BTreePage<K> *leftSiblingLink = NULL, *rightSiblingLink = NULL;
	...
	for( i = 0; i <= numOfKeys; i++ ) {
      ...
      for( i = numOfParentLink - 1; i < parentNumOfKeys - 1; ++i ) {
    ...
    for( i = numOfParentLink; i < parentNumOfKeys - 1; ++i ) {
      parentLink->keys[i] = parentLink->keys[i+1];
      parentLink->links[i+1] = parentLink->links[i+2];
      --(parentLink->links[i+1]->numOfParentLink);
    }
...
template <class K>
void BTree<K>::deleteAllPages( BTreePage<K> *page )
{
  if( page == NULL )
    return;
...
template <class K>
void BTree<K>::printAll( )
{
  BTreeIterator<K> I(*this);
  for( ; !I.done(); I.next() ) {
    cout << "Page: ";
    for( int i = 0; i < I.page->numOfKeys; i++ )
      cout << *(I.page->keys[i]) << " ";
    for( i = I.page->numOfKeys; i < I.tree.theOrder; i++ )
      cout << "  ";
...
template <class K>
bool BTreeIterator<K>::next( )
{
#if SAFETY > 0
  if( bDone )
    error("K BTreeIterator<K>::next( ) : "
...
template class BTree< Monomial<Integer> >;
template class BTree< Monomial<Rational> >;
template class BTreeIterator<Char>;
template class BTreeIterator< Monomial<Integer> >;
template class BTreeIterator< Monomial<Rational> >;
