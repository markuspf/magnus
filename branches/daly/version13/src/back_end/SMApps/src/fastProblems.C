...
{
  Word tmp = theWord.getWord();
  int len = tmp.freelyReduce().length();

  LogMessage msg( theWord );
  
  msg << "The length of " << Name( theWord ) << " in the freely reduced form is equal to " << len << ".";
  msg.send();
}
...
void ComputeRSApproximation::takeControl( )
{
  // Check that the presentation has correct form

  FPGroup G = theGroup.getFPGroup();
  if( G.namesOfGenerators()[0] != 't' ) {
    Message msg;
    msg << "Cannot start the computation. The first generator of the "
      "presentation must be 't' and the exponet sum of 't' in every "
      "relator must be 0.";
    msg.send(); 
    return; 
  }

  SetOf<Word> S1 = G.getRelators();
  ...
  // All is fine, proceed with printing relators

  RSApproximation RSA( theGroup.getFPGroup() );
  ...
  LogMessage msg( *this, theGroup );
  msg << Link( Chars("Click here to see the l's approximation of ") + 
	       Text( Name( theGroup) ),
	       "RSApproximation",
	       out.getFileName() );
  msg.send();
...
void ComputeMean::takeControl( )
{
  double m = theSet.getSet().mean();

  LogMessage msg( theSet );
  msg << "The mean of " << Name( theSet ) << " is " << m << ".";
  msg.send();
}


void ComputeDeviation::takeControl( )
{
  double d = theSet.getSet().deviation();

  LogMessage msg( theSet );
  msg << "The standard deviation of " << Name( theSet ) << " is " << d << ".";
  msg.send();
}
...
void ShowGraph::takeControl( )
{
  InvokingMessage message;
  message << "showPolynomialGraph {" << Name(poly) << "}";
  message.send();
}


void ComputePolynomialSum::takeControl( )
{
  SMObject* smo =
    new SMPolynomial( poly1.getPolynomial() + poly2.getPolynomial(),
		      Text("The sum of") + Name( poly1 )
		      + "and" + Name( poly2 )
		);
  
  ListOf<OID> dependencies( poly1 );
  dependencies.append( poly2 );
  CheckinMessage( *smo, "", dependencies ).send();
}


void ComputePolynomialProduct::takeControl( )
{
  SMObject* smo =
    new SMPolynomial( poly1.getPolynomial() * poly2.getPolynomial(),
		      Text("The product of") + Name( poly1 )
		      + "and" + Name( poly2 )
		);
