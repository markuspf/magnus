...
void NCEnumerateTotalARCer::enumerate( )
{
  ...
  while(counter < numberOfAll) {
    ...
    if (newRelator.length() > 0) {
      counter++;
      file << counter << ": ";
      theGroup.printWord(file,newRelator);
      file << endl << endl << flush;
    }
...
void NCRelatorEnumerator::start( ) {              
    ...
    if ( selArcer == Chars("RB1") ){
      int conjL;
      int factors;
      arcer = new NCEnumerateRandomARCer( *this,theGroup.getFPGroup() ); 
      fileName = ((NCEnumerateRandomARCer* )arcer)->getFileName();
