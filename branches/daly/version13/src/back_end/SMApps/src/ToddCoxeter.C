...
const PermutationRepresentation& ToddCoxeterARCer::getTransversal()const{
  if (permutationRepresentation == NULL)
    error("VectorOf<Word> getTransversal()const : "
	  "transversal was not computed yet.");
...
void ToddCoxeter::takeControl( )
{
  if ( freeARCs() > 0 )
	...
	explanation = Text("this follows from the canonical decomposition of")+
      ...
      theGroup.gic.putHaveOrder( arcer->getRetValue(),
				 explanation );
...
void SchreierTransversal::takeControl( )
{
  if (isInfinite()){
    ...
    msg <<  Name(theObject) << " is infinite. Can't compute" 
  ...
  if (haveSchreierTransvl() ) {
    ...
    for (int i1=0;i1<transversal.length();i1++){
      file << "s" << i1+1 << " = ";
      g.printWord(file,transversal[i1]);
      file << endl;
    }
    LogMessage msg(*this,theObject);
...
bool SchreierTransversal::isInfinite()const{
  if (Chars(theObject.typeID())==Chars("SMFPGroup")) {
    ...
    if (((SMSubgroup& )theObject).sic.index() == 0)
      return true;
    else
      return false;
}
...
void SchreierTransversal::viewStructure(ostream& ostr) const
{
  ...
  ProblemView pv( ostr, oid(), "Schreier representatives",
...
void PermutationRepProblem::takeControl( )
{
  if (isInfinite()){
    LogMessage msg(*this,theObject);
    msg <<  Name(theObject) << " is infinite. Can't find" 
...
PermutationRepProblem::getPermutations()const{
  if (Chars(theObject.typeID())==Chars("SMFPGroup"))
...
{
  ProblemView pv( ostr, oid(), "Permutation representation",
...
WordRepresentative::WordRepresentative(class IsSGNormal& boss)
  : theParent( boss.getSubgroup() ),
    ...
    arcerStarted( false )
{
  for( int i1 = 0; i1 < theWordsRepsFound.length(); ++i1 ) {
    theWordsRepsFound[i1] = false;
  }
  adminStart();
}
...
{
  ProblemView pv( ostr, oid(), "Schreier representative of a word",
		  Text("Find Schreier representative of ") + 
		  Name( theWord ) ,
