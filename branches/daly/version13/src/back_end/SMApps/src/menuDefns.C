...
// ----------------------------- CALCULUS --------------------------------- //
    ...
    M.add( ReadSMPolynomial(), Text("Polynomial"), DefinePolynomial() );
...
void ObjectSmith::outputToolsMenuDefns(ostream& ostr)
{

  {
    ...
    */
    ...
    M.add( ReadRSApproximationItem(),
	   Text("Compute l's approximation of") + Name(1),
	   DefineInt("Enter parameter l:", 0)
	   );
...

  {
    ...
    M.addDisabled( Text("Rewrite") + Name(1) + Text("as an n-th power") );
    ...
    M.addDisabled( Text("Compute the length of the reduced form of") 
		   + Name(1) + Text("as an element of a free abelian group") );

    M.addDisabled( Text("Compute the i-th initial segment of the reduced form of") 
		   + Name(1) + Text("as an element of a free abelian group") );

    M.addDisabled( Text("Compute the j-th terminal segment of the reduced form of") 
		   + Name(1) + Text("as an element of a free abelian group") );
    
    M.separator();

    M.addDisabled( Text("Compute the representative of") 
    ...
    M.addDisabled( Text("Rewrite") + Name(1) + Text("as a product of squares") );
    ...
    M.addDisabled( Text("Rewrite") + Name(1) + Text("as a product of commutators")
		   );
...

  {
    ...
    M.addDisabled( Text("Is") + Name(1) + Text("free abelian ?") );
    ...
    M.addDisabled( Text("Compute the first n terms for a short-lex system of representatives for the elements of") + Name(1) );
...
*/	 
...

  {
    ...
    M.addDisabled( Text("Rewrite") + Name(2) 
		   + "in terms of a canonical basis for" + Name(1) );
    ...
    M.addDisabled( Text("Compute the the canonical decomposition of") 
    ...
    M.add( Ctor2<WordInNielsenBasisSGOfFree,SMSubgroup,SMWord>(),
	   Text("Rewrite") + Name(2) + "in term of a Nielsen basis for" + Name(1)
	   );
    ...
    M.addDisabled( Text("Compute the length of") + Name(2) 
      + "when rewritten as a reduced word in a Nielsen basis for" + Name(1)
		   );
  ...
  
  {
    ...
    M.addDisabled( Text("Is") + Name(1) 
		   + "a direct factor of" + Name(2) + "?"
		   );

    M.addDisabled( Text("Is") + Name(2) 
		   + "a direct factor of" + Name(1) + "?"
		   );
    ...
    M.add( Ctor2<AbelianSGJoin,SMSubgroup,SMSubgroup>(),
	   Text("Find a canonical basis for the join of") + Name(1) 
	   + "and" + Name(2)
	   );
    
    M.add( Ctor2<AbelianSGIntersectionProblem,SMSubgroup,SMSubgroup>(),
	   Text("Find a canonical basis for the intersection of") + Name(1)
	   + "and" + Name(2)
	   );
    ...
    M.addDisabled( 
      Text("Compute the rank of the join of") 
      + Name(1) + "and" + Name(2) 
      );
 
    M.addDisabled( 
      Text("Compute the rank of the intersection of") 
      + Name(1) + "and" + Name(2) 
      );
    ...
    M.add( Ctor2<SGOfFreeJoin,SMSubgroup,SMSubgroup>(),
	   Text("Find a Nielsen basis for the join of") 
	   + Name(1) + "and" + Name(2)
	   );

    M.add( Ctor2<SGOfFreeIntersection,SMSubgroup,SMSubgroup>(),
	   Text("Find a Nielsen basis for the intersection of") 
	   + Name(1) + "and" + Name(2)
	   );
 ...
 {
   Menu1<SMDataSet> M( Menu::TOOLS );

   M.startItemGroup( );
   
   M.add( Ctor1<ComputeMean,SMDataSet>(),
	  Text("Compute the mean of") + Name(1)
	  );

   M.add( Ctor1<ComputeDeviation,SMDataSet>(),
	  Text("Compute the standard deviation of") + Name(1)
	  );
 ...
 {
   Menu1<SMPolynomial> M( Menu::TOOLS );
   

   M.startItemGroup( );
   
   M.add( Ctor1<ShowGraph,SMPolynomial>(),
	  Text("Show graph of") + Name(1)
	  );
 ...
 {
   Menu2<SMPolynomial,SMPolynomial> M( Menu::TOOLS );
   

   M.startItemGroup( );
   
   M.add( Ctor2<ComputePolynomialSum, SMPolynomial, SMPolynomial>(),
	  Text("Compute the sum") + Name(1) + "+" + Name(2)
	  );

   M.add( Ctor2<ComputePolynomialProduct, SMPolynomial, SMPolynomial>(),
	  Text("Compute the product") + Name(1) + "*" + Name(2)
	  );
