...
{ 
  if( theGroup.gic.isAutomatic() != dontknow )
    resultIsFast();
}
...
void AGProblem::takeControl( )
{
  ...
  if( theGroup.gcm().agSupervisor->state() 
      == ComputationManager::TERMINATED )
    adminTerminate();
}
...
{
  ProblemView pv( ostr, oid(), "Seek automatic structure",
		  Text("Seek a ShortLex automatic structure for")
		  + Name( theGroup ),
		  helpID( "AGProblem", theGroup ),
		  "Aut struct"
		  );

  pv.startItemGroup();

  pv.add("For the use of this problem", agSupervisor.arcSlotID(), 100);
