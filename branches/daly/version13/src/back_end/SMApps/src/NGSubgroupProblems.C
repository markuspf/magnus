...
void SGOfNGcomputeBasisProblem::viewStructure(ostream& ostr) const
{
  ...
  pv.add("For use of this problem",initializeSGOfNG.arcSlotID(), 100);
  pv.done();
}
...
void SGOfNGindexProblem::viewStructure(ostream& ostr) const
{
  ...
  pv.startItemGroup();
  pv.add("Compute the index",ThisARCSlotID(), 100);
  if( ! theSubgroup.sic.haveSGOfNGPreimageInitialized() )
    pv.add( Text("Compute a basis of the free preimage of ")+ 
	    Name(theSubgroup ),
	    initPreimageSGOfNG.arcSlotID(), 100 );
...
{
 ProblemView pv( ostr, oid(), "Compute the class",
...
{
  ProblemView pv( ostr, oid(), "Compute a presentation",
...
{
  ProblemView pv( ostr, oid(), "Find the normal closure",
