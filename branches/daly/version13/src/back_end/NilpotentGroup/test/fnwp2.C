...
int main ( )
{
  ...
  Word w;
  int i = 0;
  NilpotentGroup NG(F.numberOfGenerators(),c);
  ...
  while( 1 ) {
    ...
    bool EP = NG.wordProblem(w); 
    cout << (EP ? "yes" : "no") << "   DB: " << flush;

    bool DB = wordProblem(F,c,w);
    cout << (DB ? "yes" : "no") << endl;
    
    if( DB != EP ) {
      cout << "Different answers ! Exiting ...";
      return 0;
    }
      
    w = w.nextFreelyReduced( F.numberOfGenerators() );
  }
}
