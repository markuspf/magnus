...
BACKENDCOMPONENTS = general Elt Matrix Equations Group Subgroup KB FSA Map \
                    NilpotentGroup AProducts

# FILES:
# FILES lists all source files (without .[Cc] suffixes) which are
# part of the session manager:

FILES = ComputationManager FEData GCM GIC Menu ObjectSmith OutMessages \
        ResourceManager SMFPGroup SMMap SMObject SMSubgroup SMWord \
	SessionManager Supervisor TheObjects viewStructure SMEquation \
        ARCer MIC SMSetOfWords SMVectorOfWords
