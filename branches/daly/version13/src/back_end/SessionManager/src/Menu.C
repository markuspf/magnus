...
SMObject* ReadSMFPGroup::construct(Chars& name, istream& istr) const
{
  ...
  if ( err.length() == 0 ) {
    ...
    if ( G.getRelators().cardinality() == 1 )
      return new SMFPGroup( G,
			    "User defined one relator group",
...
SMObject* ReadSMAbelianGroup::construct(Chars& name, istream& istr) const
{
  ...
  if ( err.length() == 0 ) {
	 ...
	 SMFPGroup* smo
		= new SMFPGroup( G, "User defined abelian group", SMFPGroup::ABELIAN );
...
SMObject* ReadSMNilpotentGroup::construct(Chars& name, istream& istr) const
{
  ...
  if ( err.length() == 0 ) {
    if( G.getRelators().cardinality() > 0 ){
      ...
      SMFPGroup* smo
	= new SMFPGroup( G, "User defined nilpotent group", SMFPGroup::NILPOTENT );
      
      smo->gic.putIsNilpotent( theClass );
      return smo;
    }else {
      SMFPGroup* smo
	= new SMFPGroup( FPGroup(G.namesOfGenerators()),
...
SMObject* ReadSMORGroup::construct(Chars& name, istream& istr) const
{
  ...
  if ( err.length() == 0 ) {
    ...
    if ( G.getRelators().cardinality() == 1 ) {
      ...
      if( r.isProperPower() ) {
	return new SMFPGroup( G,
			      "User defined one relator group with torsion",
	 ...
	 if ( lambda < 6 && lambda != 0 ) {
		...
		if ( lambda == 0 )
		  ...
		  o << "User defined C'(1/"
			 ...
			 << ") group"
			 << ends;
		SMFPGroup* smo = new SMFPGroup(G, o.str(), 
					       SMFPGroup::SMALL_CANCELLATION);
		o.freeze(0);
...
SMObject* ReadSMWord::construct(SMObject* a1, Chars& name, istream& istr) const
{
  ...
  for(; !I.done(); I.next()) {
    ...
    if( Chars(smo->typeID()) == Chars("SMWord"))
      O |= I.value();
  }
  ...
  if ( err.length() == 0 ) {
    ...
    return new SMWord( parent, w,
		       Text("User defined word in the generators of")
		       + Name( parent )
		       );
...
ReadSMEqSystem::construct(SMObject* a1, Chars& name, istream& istr) const
{
  ...
  VectorOf<Word> system = ESP.parseEqSystem( G.namesOfGenerators() ,
					     newNames , err );
  ...
  if ( err.length() == 0 ) 
    {
      ...
      return new SMEqSystem( parent, FreeGroup( newNames ), system, numOfVar,
			     Text("User defined system of equations in") + Name( parent )
			     );
...
ReadSMSetOfWords::construct(SMObject* a1, Chars& name, istream& istr) const
{
  ...
  if ( err.length() == 0 ) {
    ...
    return new SMSetOfWords( parent, generators,
		 Text("User defined set of words in") + Name( parent )
		 );
...
ReadSMVectorOfWords::construct(SMObject* a1, Chars& name, istream& istr) const
{
  ...
  if ( err.length() == 0 ) {
    ...
    return new SMVectorOfWords( parent, generators,
		 Text("User defined tuple of words in") + Name( parent )
		 );
...
SMObject*
ReadSMMagnusBreakdown::construct(SMObject* a1, Chars& name,
				 istream& istr) const
{
...
SMObject*
ReadPHeight::construct(SMObject* a1, Chars& name, istream& istr)
     const
{
  int p;
  istr >> p;
...
SMObject*
ReadInitialSegmentItem::construct(SMObject* a1, Chars& name, istream& istr)
     const
{
  int p;
  istr >> p;
...
SMObject*
ReadTerminalSegmentItem::construct(SMObject* a1, Chars& name, istream& istr)
     const
{
  int p;
  istr >> p;
...
SMObject*
ReadSegmentOfWordItem::construct(SMObject* a1, Chars& name, istream& istr)
     const
{
  int start;
  istr >> start;
  ...
  if( start > smo->getWord().length() ) {
    ...
    message << "The starting position should be less than " << smo->getWord().length()+1 << ".";
    message.send();
    return 0;
  }
...
SMObject*
ReadFreeGetN_thElementItem::construct(SMObject* a1, Chars& name, istream& istr)
     const
{
  int p;
  istr >> p;
...
SMObject*
ReadFreeGetNextN_thElementItem::construct(SMObject* a1, Chars& name, istream& istr)
     const
{
  int p;
  istr >> p;
...
SMObject*
ReadIsSGNilpotentItem::construct(SMObject* a1, Chars& name,
				 istream& istr) const
{
  int p;
  istr >> p;
...
SMObject*
ReadLCStermProblem::construct(SMObject* a1, Chars& name,
			      istream& istr) const
{
  int p;
  istr >> p;
...
SMObject*
ReadRSApproximationItem::construct(SMObject* a1, Chars& name, istream& istr)
  const
{
  int p;
  istr >> p;
  return new ComputeRSApproximation( *(SMFPGroup*)a1, p );
}
...
SMObject*
ReadPackage::construct(Chars& name,
		       istream& istr) const
{
  Chars tname;
  Chars command;
  ...
  for (int i=0;i<nOfParam;i++){
    ...
    if (ptype == Chars("Integer"))
      pt = ParamRecord::INT;
    else if (ptype == Chars("String"))
      pt = ParamRecord::STRING;
    else
      pt = ParamRecord::BOOL;
...
SMObject*
ReadEditPackage::construct(Chars& name,
			   istream& istr) const
{
  Type::notype n;
  Type id(n);
...
SMObject*
ReadGroupPackageID::construct(SMObject* a1,
			 Chars& name,
			 istream& istr) const
{
  int id;
...
SMObject*
ReadWordPackageID::construct(SMObject* a1,
			     Chars& name,
			     istream& istr) const
{
  int id;
...
SMObject*
ReadSubgroupPackageID::construct(SMObject* a1,
				 Chars& name,
				 istream& istr) const
{
  int id;
...
SMObject*
ReadMapPackageID::construct(SMObject* a1,
			    Chars& name,
			    istream& istr) const
{
  int id;
...
SMObject*
ReadHomoPackageID::construct(SMObject* a1,
			    Chars& name,
			    istream& istr) const
{
  int id;
...
SMObject*
ReadSMDataSet::construct( Chars& name, istream& istr) const
{
  istr >> name;
  ...
  if ( err.length() == 0 ) {
    ...
    return new SMDataSet( S, Text("User defined data set") );
...
SMObject*
ReadSMPolynomial::construct( Chars& name, istream& istr) const
{
  istr >> name;
  ...
  if ( err.length() == 0 ) {
    ...
    return new SMPolynomial( P, Text("User defined polynomial") );
...
void Menu::DefinePolynomial::print(ostream& ostr) const
{
  ostr << "_definePolynomial_ {Define a polynomial}";
}
