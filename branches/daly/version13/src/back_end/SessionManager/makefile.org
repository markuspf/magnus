...
## Contents: GNU Makefile for the session manager
##
...
BACKENDCOMPONENTS = libg++ general Elt Matrix Polynomial Equations Group Apps Subgroup KB FSA Map Genetic NilpotentGroup AProducts Statistics Calculus GAP Todd-Coxeter Enumerators Packages SMApps

# FILES:
# FILES lists all source Files (without .[Cc] suffixes) which are
# part of the session manager:

FILES = GIC SM1 DatabaseManager Property BaseProperties registerProperties \
	SMSubgroup SMEquation ObjectFactory hacks RandomDefinitionsGenerator \
	SMDataSet SMPolynomial

_FILES = ComputationManager FEData GCM Menu ObjectSmith OutMessages \
        ResourceManager SMFPGroup SMMap SMObject SMWord \
	SessionManager Supervisor TheObjects viewStructure \
        ARCer MIC SMSetOfWords SMVectorOfWords SMList SMEnumerator  \
	ViewContents SMEqSystem SMPermutation
...
catlib:	
	-rm -f src/SM1.C; $(CONCAT) $(foreach file,$(_FILES),src/$(file).C) > src/SM1.C
