// Contents: Implementation of heapsort
//
...
//
// pre:  the heap is build and node i exist, d >= 2
// post: if (b < heapSize) A[maxInd] = max{A[i], A[a],...,A[b]}
//       else A[maxInd] = max{A[i], A[a],...,A[heapSize-1]}
//       ( a = d*i+1, b = d*i+d )
{
  int maxInd = i;
  int a = d*i+1, b = d*i+d;
  if( a > heapSize ) return maxInd;
  int j = a;

  // invariant: 
  // j <= heapSize && a <= j <= b+1 &&
  // A[maxInd] = max{A[i], A[a],...,A[j-1]} 

  while( j < heapSize && j <= b ) {
    if( A[maxInd] < A[j] ) 
      maxInd = j;
    ++j;
  }
