...
int parent( int d, int i ) { return (i-1)/d; }
// return parent of i if i > 0

void insert( int* A, int& heapSize, int d, int key )
// pre:  the heap is build
// post: key is inserted into the heap and the heap is updated
{
  ++heapSize;
  int i = heapSize-1;
  
  // invariant:
  // i >= 0 && for any x, x from Children(i) -> key >= x
  while( i > 0 && A[parent(d,i)] < key ) {
    A[i] = A[parent(d,i)];
    i = parent(d,i);
  }
  A[i] = key;
