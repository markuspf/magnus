/*
 *   $Id: DList.h 4 2005-01-07 00:28:20Z daly $
 */
...
public:
  ...
  DListOf sublist( int i, int j ) const {
	 return DListOf( look()->sublist(i, j) );
  }
  ...
  DListOf reverse( ) const { return DListOf( look()->reverse() ); }
  // Returns a copy of this list in reverse order.

  void append( const T& t ) { change()->insert(t, length()); }
  // Appends t to this List.

  void prepend( const T& t ) { change()->insert(t, 0); }
  // Prepends t to this List.

  void insert( const T& t, int i ) { change()->insert(t, i); }
  ...
  void insert( const DListOf& L, int i ) { change()->insert( L.look(), i ); }
  ...
  void removeElement( const T& t ) { change()->removeElement(t); }
  // Remove t from this list. It is not an error if t is not in this list.

  void removeElementOfIndex( int i ) { change()->removeElementOfIndex(i); }
  ...
  void splice( const DListOf& L, int i, int j ) {
         if ( L.look() == look() ) { 
	        DListOf<T> aCopy = L;
	        change()->splice( aCopy.look(), i, j );
         }
	 else change()->splice(L.look(), i, j);
  }
...
public:
  ...
  Bool previous( ) { return change()->previous(); }
  // Advances this iterator.
  // Returns TRUE iff the iteration has not finished.
  // This may be called after it returns FALSE with no side effect.
  ...
  Bool valid( ) const { return !look()->done(); }
