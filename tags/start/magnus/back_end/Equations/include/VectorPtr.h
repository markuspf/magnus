/*
 *   $Id: VectorPtr.h 4 2005-01-07 00:28:20Z daly $
 */
...
public:
  ...
  {
    if (look() == v.look()) return true;
    if ( look()->length() != v.look()->length() ) return false;
    int i = look()->length();
    while ( i-- ) 
      if( look()->isValid(i) != v.look()->isValid(i) ||
	  look()->isValid(i) && !(look()->val(i) == v.look()->val(i)) ) 
	return false;
    return true;
  }
  ...
  VectorItemRef<T> operator [] ( int i ) { return change()->operator [](i); }

  T val( int i ) const { return look()->val(i); }

  T& ref( int i ) { return change()->ref(i); }

  bool isValid( int i ) const { return look()->isValid(i); }
  ...
  int indexOf( const T& t ) const {
    int i = length();
    while ( i-- ) 
      if ( look()->isValid(i) && look()->val(i) == t ) return i;
    return -1;
  }
