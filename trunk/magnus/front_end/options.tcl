##
##   $Id: options.tcl 232 2007-04-02 18:41:56Z ybryukhov $
##
...
proc setFontSize {sz} {
        ...
        set hf [list -family Helvetica -size $sz]
        set cf [list -family Courier -size $sz]
   ...
   option add *Message.font $hf

        global titleFont linkFont
        set titleFont [list -family Times -size [expr $sz + 4]]
        set linkFont [list -family Times -size $sz -slant italic]
}

proc changeFontSize {sz} {
  ...
  setFontSize $sz
  changeFontSize_aux . $sz

  workspace_changeNameFontSize $sz
  setHeaderTag
}

proc changeFontSize_aux {root sz} {
        global titleFont linkFont

  foreach w [winfo children $root] {
          catch {
                  $w configure -font [newFont $w $sz]
        $w tag configure title -font $titleFont
        $w tag configure link -font $linkFont
     }

     changeFontSize_aux $w $sz
  }
}
...
proc newFont {w sz} {
        ...
        array set f [option get $w font {}]
        array set f [list -size $sz]
        return [array get f]
}
