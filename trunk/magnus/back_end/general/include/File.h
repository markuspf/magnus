...
struct File : public std::fstream
{
  ...
  File( )  
  {
    ...
    free(tmpFileName);
    open(theFileName, std::ios::app|std::ios::out);
    colorFile.open(theFileName+Chars(".color"),std::ios::app|std::ios::out);
#if SAFETY > 0
    if( !(*this) )
      error( "File::File( ) : "
	     "Cannot open a file." );
  ...
  File( const Chars& fileName ) : 
    std::fstream( fileName, std::ios::app|std::ios::out ), 
...
private:
  ...
  std::fstream colorFile; // holds color mapping
};


inline std::ostream& ready(std::ostream& ostr) 
{
  ostr <<  std::flush;
  return ostr;
}

inline std::ostream& end(std::ostream& ostr) 
{
  ostr <<  std::flush;
  return ostr;
}

void dumpstream(std::istream& src, std::ostream& trg);
#endif  // _File_h_
