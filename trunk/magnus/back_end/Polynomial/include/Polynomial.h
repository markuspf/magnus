...
private:
  ...
  virtual void theKeyIsFound( const Monomial<R>& key, Monomial<R>& value )
  {
    ...
    if( value.getCoefficient() == R(0) )
		 // I need this-> here because otherwise
		 // GCC 4.1 does not see remove()
		 // probably some kind of template instantiation is needed
		 // There was no such problem with GCC 4.0
      this->remove(value);
