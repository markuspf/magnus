...
#if SAFETY > 0
  if( !dataFile )
      error( Chars("SMListData::openFiles( int ) : Cannot open the data file: ") + dataFileName);
#endif    
...
#if SAFETY > 0
  if( !refFile )
      error( Chars("SMListData::openFiles( int ) : Cannot open the referencies file: ") + refFileName);
#endif 
}
