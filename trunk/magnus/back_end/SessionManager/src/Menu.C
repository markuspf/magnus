...
SMObject* ReadSMSmallCancGroup::construct(Chars& name, istream& istr) const
{
  ...
  if ( err.length() == 0 ) {
    ...
    if ( lambda < 6 && lambda != 0 ) {
      ...
      o << "0 {This group is only C'(1/" << lambda << ").}";
      ...
      if ( lambda == 0 )
			...
			o << "User defined C'(1/" << lambda << ") group";
...
SMObject*
ReadAffineItem::construct(SMObject* a1, Chars& name, istream& istr) const
{
  int d;
  istr >> d;
  return new AffineSupervisor( *(SMFPGroup*)a1, d );
}
