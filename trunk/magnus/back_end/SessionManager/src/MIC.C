...
void MIC::putDoesExtendToHom( OID compManager, bool doesExtend, Chars explanation )
{
  if( doesExtendToHom() != dontknow ) return;
  putProperty( ExtendToHomProperty(explanation), doesExtend );
  LogMessage msg(compManager, mapOID);
  msg << getExtendToHomMessage(); //<< " (from MIC::putDoesExtendToHom)";
  msg.send();
}
