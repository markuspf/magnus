...
// and not their values
  ...
  inline friend std::ostream& operator <<
        ( std::ostream& ostr, const DFSARep& D ) {
        ostr << "This is just a dummy operator"<< std::endl;
    return ostr;
  }
...
// that, and if they don't match the DFSA's cannot be equal.
  ...
  virtual void printAccepting(std::ostream &str=std::cout) const ;
  void printFlags(std::ostream &str=std::cout) const ;
  virtual void printStates(std::ostream &str=std::cout) const ;
  void printTableDensely(std::ostream &str=std::cout) const ;
  void printTableSparsely(std::ostream &str=std::cout) const ;
...
//  Bool nextAcceptedWord(Word w, int*& history) const {}; //@rn bad arg
  void readFrom(std::istream &str = std::cin);
  void printOn(std::ostream &str = std::cout) const;
  void GAPprintWord(std::ostream &str,const Word & w) const;
  void printAlphabet(std::ostream &str=std::cout) const ;
  void printWord(std::ostream &str,const Word & w) const ;
...
// Many of these are virtual because they are overwritten in GroupDFSA
  ...
  virtual void setGeneratorNames(const VectorOf<Chars> & names) {
    if (generatorNames!=0)
      std::cerr <<"Cannot set generator names because they're already set."<<std::endl;
