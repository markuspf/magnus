...
Trichotomy IsTrivialChecker::preliminaryCheckings()
{
  ...
  if( checker.existsGenWithExpSumZeroInEveryRelator(g) ) {
    ...
    msg << " is equal to 0 in every relator of the group";

    explanation = msg.str().c_str();
    return no;
  }
  ...
  for( int i = 0; i < G.numberOfGenerators(); ++i ) {
    ...
    if( gcd > 1 ) {
      ...
      msg << " in the group relations is " << gcd;
...
void IsTrivialProblem::takeControl( )
{
  if ( ! displayInFE() || freeARCs() > 0 ) {
	...
	msgTmp
		<< ": because it's not trivial in lower central quotient of class " 
	   << retClass;
	Chars explanation = msgTmp.str().c_str();
