...
//
{
  ...
  MultiDimArray<Word> pop(numOfEq,numOfVars,popSize),newPop(numOfEq,numOfVars,popSize);
  Matrix<int> fit(numOfEq,popSize);
  
  float crossRate = cfg.chanceOfCrossover();
  float mutRate = cfg.chanceOfMutation();
  int max, min, minInd, g;
  Matrix<int> fitInd(numOfEq,popSize);
 
  // create the original random populations
  for( int l = 0; l < numOfEq; ++l )
    for( int k = 0; k < numOfVars; ++k )
      for( int i = 0; i < popSize; ++i ) {
	pop.get(l,k,i) = randomWord();
      }
  ...
  for( g = 0; true; ++g ) {
	...
	if( g == 0 || !cfg.haveStrongElitistSelection() ) {
	  for( int k = 0; k < numOfVars; ++k )
	    M[l].setGeneratingImages(k,pop.get(l,k,i));
	...
	*/
	...
	for( int k = 0; k < numOfVars; ++k )
	  S.setGeneratingImages(k,pop.get(l,k,minInd));
	...
	for( int i = 0; i < popSize; ++i ) {
	  if( r.rand() <= crossRate ) {
	    ...
	    newPop.get(l,k,i) = crossover(pop.get(l,k,i1),pop.get(l,k,i2));
	  }
	  else {
	    newPop.get(l,k,i) = pop.get(l,k,i);
	  }
	}
	...
	for( int i = 0; i < popSize; ++i ) {
	  if( r.rand() <= mutRate ) {
	    newPop.get(l,k,i) = mutate(newPop.get(l,k,i));
	  }
	}
	...
	VectorOf<int> newFit(popSize), newFitInd(popSize);
	
	for( int i = 0; i < popSize; ++i ) {
	  for( int k = 0; k < numOfVars; ++k )
	    M[l].setGeneratingImages(k,newPop.get(l,k,i));
	...
	for( int i = 0; i < popSize; ++i ) 
	  for( int j = 0; j < popSize; ++j ) 
	    if( newFit[i] < fit[l][j] ) {
	      ...
	      for( int k = 0; k < numOfVars; ++k )
				pop.get(l,k,j) = newPop.get(l,k,i);
	      break;
	    }
	/*
	...
	*/
	...
	for( int k = 0; k < numOfVars; ++k )
	  if( cfg.haveElitistSelection() ) {
	    newPop.get(l,k,0) = pop.get(l,k,minInd);
	  }
	
	// prepare for the next iteration
	for( int k = 0; k < numOfVars; ++k )
	  for( int i = 0; i < popSize; ++i ) {
	    pop.get(l,k,i) = newPop.get(l,k,i);
	  }    
      }
    }
  }
