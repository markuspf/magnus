...
public:
  ...
  friend std::ostream& operator < ( std::ostream& ostr, const Generator& g )
  {
    ostr < g.genrep;
    return ostr;
  }
  
  friend std::istream& operator > ( std::istream& istr, Generator& g )
  {
    istr > g.genrep;
    return istr;
  }
...
inline std::ostream& operator << ( std::ostream& ostr, Generator g ) {
	ostr << ord(g);
	return ostr;
}
